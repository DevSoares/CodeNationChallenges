...
namespace Codenation.Challenge.Services
{
    public class ChallengeService : IChallengeService
    {
        ...
        {
            List<int> acceletarionIds = CodenationContext.Candidates.Where(c => (c.AccelerationId == accelerationId) && (c.UserId == userId)).Select(a => a.AccelerationId).ToList();

            List<Acceleration> accelerations = CodenationContext.Accelerations.Where(a => acceletarionIds.Contains(a.Id)).ToList();
