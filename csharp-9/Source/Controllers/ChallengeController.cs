...
namespace Codenation.Challenge.Controllers
{
    ...
    public class ChallengeController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<ChallengeDTO>> GetAll(int? accelerationId = null, int? userId = null)
        {
            ...
            {
                List<ChallengeDTO> challengeDTOs = _challengeService.FindByAccelerationIdAndUserId(accelerationId.Value, userId.Value)
                                                                    .Select(x => _mapper.Map<ChallengeDTO>(x)).ToList();
                return Ok();
            }
            else
                return NoContent();
