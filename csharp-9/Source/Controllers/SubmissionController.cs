...
namespace Codenation.Challenge.Controllers
{
    ...
    public class SubmissionController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<SubmissionDTO>> GetAll(int? challengeId = null, int? accelerationId = null)
        {
            ...
            {
                List<SubmissionDTO> submissionDTOs = _submissionService.FindByChallengeIdAndAccelerationId(challengeId.Value, accelerationId.Value)
                                                                        .Select(x => _mapper.Map<SubmissionDTO>(x)).ToList();
